...
const ViewClientUpcomingEvent = ({event}: {event: Event}) => {
    const bodyClass = clsx("grid grid-rows-3 grid-cols-[86px_1fr_1fr_1fr] gap-x-2 mt-1 font-body rounded-md py-1",);
    const buttonClass = clsx("row-span-3 font-header !px-2")
    return (
        <div className={bodyClass}>
            <LinkButton to={`/event/${event.id}`} state={{event: event}} className={buttonClass} variant="primary">{event.id}</LinkButton>
            <div>Begin</div>
            <div>End</div>
            <div>Type</div>
